...
d_src = Path(f"{os.getenv('HOME')}/.cargo/registry/src")
d_crates = Path(
    d_src
    / [d for d in os.listdir(d_src) if re.compile(r"^index\.crates\.io").match(d)][0]
)
...
def parse_toml(path: Path) -> Dict[str, Any]:
    ...
    with open(path, "rb") as f:
        ...
        return {
            ...
            "text": {"copy": pkg["name"], "largetype": pkg["description"]},
            "quicklookurl": url,
        }
...
crates = [
    parse_toml(toml)
    for toml in [Path(d_crates / d / "Cargo.toml") for d in os.listdir(d_crates)]
]
